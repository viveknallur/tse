...
Based on the preference functions given (equation \ref{pref_ssl}, \ref{pref_framerate} and \ref{pref_latency}), we can calculate the relative values of the 4 CandidateServices, as given Table \ref{tbl:outranking_table}
	\begin{table}[htbp]
		\centering
...
\subsubsection{Class and Package Diagram} The class diagram shows the static relationships between the main entities in clobmas. The \textit{Bid} and \textit{Ask} interfaces shown in Figure~\ref{fig:class_diagram} are implemented by the BuyerAgent and SellerAgent respectively, with the \textit{MarketAgent} implementing both of them. The packages with the stereotype ``subsystems'', marked as Nova and Glance, belong to OpenStack.
...
We document the communication steps of our mechanism through AUML diagrams.\\
...
More figures showing the various scenarios of communication between the agents are shown in \autoref{auml_appendix}.
...
The increase in QoS attributes causes Time-to-MSR rise in a biquadratic way, as opposed to increase in number of Markets (\autoref{eq:mkt_vs_qos} and \autoref{eq:qos_vs_mkt}). 
...
\begin{enumerate}
    \item \textbf{Decomposition of Constraints}: Depending on the type of QoS constraints, the time complexity changes. If the constraints are boolean or categoric, then the time complexity for $k$ constraints in an application with $n$ AbstractServices, is $O(n \cdotp k)$. However, if the constraints are numeric, then the complexity of decomposition increases. Since each market is queried for its historical data of transactions. The complexity of decomposition now becomes $O(n \cdotp k^{2} \cdotp m) + O(SWR)$, where $m$ is the number of markets and $O(SWR)$ is the complexity of applying \textit{Stochastic Workflow Reduction} on the application structure.
...
\begin{appendix}\label{auml_appendix}
