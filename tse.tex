...
\usepackage{hyperref}
\usepackage{booktabs}
\usepackage[table]{xcolor} % for highlighting rows insTables
...
%Investigating the effect of market conditions (distributions of demand and supply) on the self-adaptive mechanism is slightly tricky. Conditions like paucity of supply, merely has the effect of less applications being able to satisfy their QoS requirements. The conditions that we investigate are shown in Table \ref{MarketConditions}. As can be seen from thesTable, the changing of QoS attribute demand and/or supply, from a uniform distribution to a skewed (bimodal) distribution does not seem to adversely affect the performance, at the aggregate level. More investigation, however, is required to establish the limits at which the aggregate performance starts to degrade noticeably.
%
%\begin{table*}[ht]
% \begin{tabular}{| c | l | l |}
% \hline
...
\begin{figure}[htbp]
	...
	\includegraphics[clip, trim=5cm 12cm 2cm 6cm, scale=0.32]{graphs/1_3_5_10_task_per_svc_scaling.pdf}
	\caption{CandidateServices increase \label{fig:task_per_svc}}
	\end{minipage}
	\begin{minipage}[b]{0.4\linewidth}
			\includegraphics[clip, trim=2cm 12cm 2cm 6cm, scale=0.32]{graphs/10_30_50_svc_per_task_scaling.pdf}
...
\begin{figure}[htbp]
	\centering
	\includegraphics[clip, trim=3cm 11cm 2cm 7cm,scale=0.47]{graphs/scaling_time_task_candidates.pdf}
...
As the number of applications that use clobmas increase, it will have to make a decision on whether to increase the number of CandidateServices, or increase the number of QoS attributes. Regardless of the business implications of each, both will have an impact on the scalability of clobmas. In the following figures (see Figure~\ref{fig:svc_per_qos} and \ref{fig:qos_per_svc}), we see that each variable affects the time-to-adapt in approximately the same way. There is a slight difference in the slope of the lines, when QoS attributes are increased (as shall be seen in Figure~\ref{fig:svc_and_qos_scaling}).

\begin{figure}[htbp]
	\begin{minipage}[b]{0.4\linewidth}
		\includegraphics[clip, trim=4cm 12cm 2cm 6cm, scale=0.32]{graphs/5_10_15_20_svc_per_qos_scaling.pdf}
		\caption{QoS increase \label{fig:svc_per_qos}}
	\end{minipage}
	\hspace{0.9cm}
	\begin{minipage}[b]{0.4\linewidth}
			\vspace{0.3cm}
		\includegraphics[clip, trim=2cm 12cm 2cm 6cm, scale=0.32]{graphs/1_3_5_7_9_qos_per_svc_scaling.pdf}
...
\begin{figure}[htbp]
\centering
\includegraphics[clip, trim=2cm 9cm 2cm 11cm, scale=0.45]{graphs/scaling_time_svcs_qos.pdf}
...
\begin{figure}[htbp]
		\begin{minipage}{0.4\linewidth}
			\includegraphics[clip, trim=3cm 14cm 5cm 6cm, scale=0.3]{graphs/1_3_5_7_10_svc_per_mkt_scaling.pdf}
			\caption{CandidateServices increase\label{fig:svc_per_mkt}}		
		\end{minipage}
		\hspace{0.9cm}
		\begin{minipage}{0.4\linewidth}
			\includegraphics[clip, trim=2cm 14cm 6cm 6cm, scale=0.3]{graphs/1_3_5_7_10_mkts_per_svc_scaling.pdf}
			\caption{Markets increase\label{fig:mkt_per_svc}}
		\end{minipage}		
\end{figure}

We see from Figure~\ref{fig:svc_per_mkt} and Figure~\ref{fig:mkt_per_svc} that the slopes of the lines is almost linear. That is, increasing the number of CandidateServices \textit{vis-a-vis} increasing the number of markets does not make a significant difference to the time-to-adapt.
...
\begin{figure}[htbp]
%		\begin{minipage}{0.4\linewidth}
			\includegraphics[clip, trim=2cm 16cm 6cm 4cm, scale=0.55]{graphs/1_4_7_9_mkts_per_qos.pdf}
			\caption{Markets increase per QoS\label{fig:mkt_per_qos}}		
\end{figure}
%		\hspace{0.9cm}
%		\begin{minipage}{0.4\linewidth}
\begin{figure}
			\includegraphics[clip, trim=2cm 16cm 5cm 4cm, scale=0.55]{graphs/2_4_6_qos_per_mkt_scaling.pdf}
...
\begin{figure}[htbp]
	\centering
	\includegraphics[clip, trim=2cm 12cm 2cm 5cm, scale=0.45]{graphs/scaling_time_qos_mkts.pdf}
